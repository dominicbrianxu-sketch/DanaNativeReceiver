...
/**
 * V25 10-POINT SELF-CALIBRATING ENVELOPE TAP + PERMANENT LEARNED COORDINATE + FULL DEBUG TRACE.
 *
...
public class DanaAutoClickAccessibilityService extends AccessibilityService {
    public static final String TAG = "DanaAutoClickV25";
    ...
    private static final Pattern RUPIAH_AMOUNT_PATTERN = Pattern.compile("(?i)\\brp\\s*[0-9][0-9\\.,]*\\b");
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        if (DANA_PACKAGE.equals(pkg) && hasPendingClaim() && danaOpenAt == 0L) {
            danaOpenAt = lastEventAt;
            scheduleDanaTimeout();
        }
        String text = eventText(event);
        traceAccessibilityEvent(event, pkg, text);

        if (DANA_PACKAGE.equals(pkg) && hasPendingClaim() && (directTapSequenceActive || awaitingClaimResult) && detectClaimAmountSuccess(text)) {
        ...
        if (DANA_PACKAGE.equals(pkg) && hasPendingClaim()) {
            ...
            if (danaOpenAt == 0L) {
                danaOpenAt = danaNow;
                danaLoadingWaitLogged = false;
                DebugTrace.detail(this, "DANA visit armed from Accessibility event; starting adaptive tap timer");
            }
        ...
        if (DANA_PACKAGE.equals(pkg) && awaitingClaimResult) {
            ...
            if (containsAny(normalized, FAILURE_TEXTS) && !directTapSequenceActive) {
    ...
    private void traceAccessibilityEvent(AccessibilityEvent event, String pkg, String text) {
        ...
        CharSequence sourceId = event.getSource() == null ? null : event.getSource().getViewIdResourceName();
        if (sourceId != null && !sourceId.toString().isEmpty()) {
            DebugTrace.detail(this, "Event sourceId=" + shortValue(sourceId.toString(), 120));
        }
    }
    ...
    /**
     * TEMPORARY DEBUG HELPER (remove once the tap issue is confirmed fixed).
     * Shows an on-screen Toast and a short vibration so tap flow can be
     * diagnosed visually on a device without adb/logcat access.
     */
    private void debugSignal(String msg) {
        Log.i(TAG, "DEBUG: " + msg);
        handler.post(() -> {
            try {
                Toast.makeText(getApplicationContext(), "DBG: " + msg, Toast.LENGTH_SHORT).show();
            } catch (Exception ignored) {}
            try {
    ...
    private void tryClickDanaButton() {
        ...
        // V26: TAP 1 does not require the Accessibility hint.
        if (danaOpenAt == 0L) { danaOpenAt = now; scheduleDanaTimeout(); }
        long danaElapsed = now - danaOpenAt;
        if (danaElapsed >= DIRECT_TAP_START_DELAY_MS) {
            DebugTrace.detail(this, "V26: starting direct tap; accessibilityHint=" + kagetUi);
    ...
    private void scheduleDirectEnvelopeTap() {
        ...
        if (danaOpenAt > 0L && now - danaOpenAt >= DANA_MAX_OPEN_MS) {
            timeoutAndCloseDana();
            return;
        }
            ...
            dispatched = dispatchGesture(gesture, new GestureResultCallback() {
                @Override public void onCompleted(GestureDescription g) {
                    ...
                    handler.postDelayed(() -> {
                        ...
                        // V26: hint disappearance is NOT success. Continue until Rp... is found.
                        if (detectAndConfirmImmediateClaimResult()) {
                            directTapSequenceActive = false;
                            return;
                        }
    ...
    private boolean detectAndConfirmImmediateClaimResult() {
        ...
        try {
            ...
            if (containsAny(text, SUCCESS_TEXTS)) {
    ...
    private void checkDanaRootForClaimResult() {
        ...
        try {
            ...
            DebugTrace.detail(this, "Claim result scan: textLength=" + allText.length() + " successMatch=" + containsAny(allText, SUCCESS_TEXTS) + " failureMatch=" + containsAny(allText, FAILURE_TEXTS));
    ...
    private void claimConfirmed(boolean success, String uiText) {
        ...
        if (success) {
            ...
            closeDanaPage("claim success / Rp detected");
    ...
    private boolean containsRupiahAmount(String text) {
        return text != null && !text.isEmpty() && RUPIAH_AMOUNT_PATTERN.matcher(text).find();
    }

    private boolean detectClaimAmountSuccess(String text) {
        if (!containsRupiahAmount(text)) return false;
        DebugTrace.success(this, "V26 SUCCESS: nominal Rupiah terdeteksi -> STOP TAP");
        return true;
    }
    ...
    private void scheduleDanaTimeout() {
        ...
        handler.postDelayed(() -> {
            ...
            if (elapsed >= DANA_MAX_OPEN_MS) timeoutAndCloseDana();
            else scheduleDanaTimeout();
        }, DANA_MAX_OPEN_MS);
    }

    private void timeoutAndCloseDana() {
        DebugTrace.warn(this, "V26: DANA >120s -> STOP TAP + CLOSE X + STANDBY");
        armed = false; awaitingClaimResult = false; clickInProgress = false;
        directTapSequenceActive = false; directTapScheduled = false;
        directTapAttempts = 0; directTapStartedAt = 0L; clickedAt = 0L;
        danaTimeoutScheduled = false;
        handler.removeCallbacksAndMessages(null);
        getSharedPreferences("dana_receiver", MODE_PRIVATE).edit().remove("pending_dana_url").apply();
    ...
    private void closeDanaPage(String reason) {
        ...
        try {
            ...
            AccessibilityNodeInfo root = getRootInActiveWindow();
            if (root != null && isFromDana(root)) {
                AccessibilityNodeInfo close = findDanaCloseNode(root, width, height);
                if (close != null) {
                    boolean clicked = false;
                    try { clicked = close.performAction(AccessibilityNodeInfo.ACTION_CLICK); } catch (Exception ignored) {}
                    try { close.recycle(); } catch (Exception ignored) {}
                    try { root.recycle(); } catch (Exception ignored) {}
                    if (clicked) return;
                } else { try { root.recycle(); } catch (Exception ignored) {} }
            }
            Path path = new Path();
            path.moveTo(Math.max(24f, width * 0.065f), Math.max(24f, height * 0.062f));
            GestureDescription gesture = new GestureDescription.Builder().addStroke(new GestureDescription.StrokeDescription(path, 0, CLOSE_GESTURE_DURATION_MS)).build();
            boolean dispatched = dispatchGesture(gesture, new GestureResultCallback() {
                @Override public void onCompleted(GestureDescription g) { DebugTrace.success(DanaAutoClickAccessibilityService.this, "V26: X DANA closed; " + reason); }
                @Override public void onCancelled(GestureDescription g) { try { performGlobalAction(AccessibilityService.GLOBAL_ACTION_BACK); } catch (Exception ignored) {} }
            }, handler);
            if (!dispatched) performGlobalAction(AccessibilityService.GLOBAL_ACTION_BACK);
        } catch (Exception e) { try { performGlobalAction(AccessibilityService.GLOBAL_ACTION_BACK); } catch (Exception ignored) {} }
    }
    ...
    private AccessibilityNodeInfo findDanaCloseNode(AccessibilityNodeInfo node, int width, int height) {
        ...
        try {
            ...
            boolean topLeft = r.centerX() <= width * 0.25f && r.centerY() <= height * 0.18f;
            boolean closeLabel = t.equalsIgnoreCase("x") || d.equalsIgnoreCase("x") || t.equalsIgnoreCase("close") || d.equalsIgnoreCase("close") || t.equalsIgnoreCase("tutup") || d.equalsIgnoreCase("tutup") || id.contains("close") || id.contains("back");
            if (topLeft && closeLabel && (node.isClickable() || node.isFocusable())) return AccessibilityNodeInfo.obtain(node);
            for (int i=0;i<node.getChildCount();i++) {
                AccessibilityNodeInfo child=null; try { child=node.getChild(i); } catch(Exception ignored) {}
                if(child!=null){ AccessibilityNodeInfo found=findDanaCloseNode(child,width,height); try{child.recycle();}catch(Exception ignored){} if(found!=null)return found; }
            }
        } catch(Exception ignored) {}
        return null;
    }
